# 📖 项目指南

> VLM-VLA Agent 技术文档 - 架构、模块和实现细节

---

## 📋 目录

1. [项目架构](#项目架构)
2. [核心模块](#核心模块)
3. [数据流程](#数据流程)
4. [训练流程](#训练流程)
5. [配置详解](#配置详解)
6. [性能优化](#性能优化)
7. [扩展开发](#扩展开发)

---

## 项目架构

### 系统概览

```
┌─────────────────────────────────────────────────────────┐
│                    VLM-VLA Agent                        │
│                                                         │
│  输入: 双时相影像 + 变化描述文本                          │
│                       ↓                                 │
│  [CLIP 视觉编码器] ─→ 视觉特征 (512维)                   │
│                       ↓                                 │
│  [特征投影层] ─→ LLM 嵌入空间 (1024维)                   │
│                       ↓                                 │
│  [Qwen LLM + LoRA] ─→ 语言理解                          │
│                       ↓                                 │
│  [多模态融合] ─→ 融合表示                                │
│                       ↓                                 │
│  [动作预测头] ─→ [中心x, 中心y, 尺度]                    │
│                                                         │
│  输出: 归一化动作向量 [0, 1]³                            │
└─────────────────────────────────────────────────────────┘
```

### 关键技术栈

| 组件 | 技术 | 版本 |
|------|------|------|
| 深度学习框架 | PyTorch | 2.0+ |
| 视觉编码器 | CLIP ViT-B/32 | OpenAI |
| 语言模型 | Qwen2.5-0.5B | 阿里云 |
| 微调方法 | LoRA | PEFT |
| 量化 | 4-bit | BitsAndBytes |
| 数据集 | LEVIR-CC | Arrow 格式 |

---

## 核心模块

### 1. config.py - 配置管理

**功能**:
- 自动环境检测（Kaggle vs 本地）
- 路径映射和管理
- 超参数配置

**关键类**:
```python
class Config:
    # 环境检测
    IS_KAGGLE = os.path.exists("/kaggle/input")

    # 路径配置
    if IS_KAGGLE:
        DATASET_PATH = "/kaggle/input/levir-cc-dateset/LEVIR-CC"
        CLIP_PATH = "/kaggle/input/clip-vit-b32"
        LLM_PATH = "/kaggle/input/qwen2.5-0.5b"
    else:
        # 本地路径
        DATASET_PATH = "data/"
        CLIP_PATH = "models/clip-vit-b32"
        LLM_PATH = "models/qwen2.5-0.5b"

    # 训练超参数
    MAX_EPOCHS = 10
    BATCH_SIZE = 4
    LEARNING_RATE = 1e-4

    # LoRA 配置
    LORA_R = 8
    LORA_ALPHA = 16
    LORA_DROPOUT = 0.05
```

**使用方式**:
```python
from src.config import Config

# 打印配置
Config.print_config()

# 验证路径
Config.verify_paths()
```

---

### 2. dataset.py - 数据加载

**功能**:
- 加载 HuggingFace Arrow 格式数据
- 自动检测数据结构
- 图像预处理和增强
- 边界框转换为动作向量

**关键类**:
```python
class LevirCCActionDataset(Dataset):
    def __init__(self, dataset_split, image_size=224):
        # 初始化数据集
        self.dataset = dataset_split
        self.image_transform = transforms.Compose([
            transforms.Resize((image_size, image_size)),
            transforms.ToTensor(),
            transforms.Normalize(
                mean=[0.48145466, 0.4578275, 0.40821073],  # CLIP 标准化
                std=[0.26862954, 0.26130258, 0.27577711]
            )
        ])

    def __getitem__(self, idx):
        # 返回数据样本
        return {
            'image_t1': Tensor(3, 224, 224),      # 时间1影像
            'image_t2': Tensor(3, 224, 224),      # 时间2影像
            'caption': str,                        # 变化描述
            'action_vector': Tensor(3,),           # [cx, cy, scale]
            'bbox': [x1, y1, x2, y2],            # 原始边界框
        }
```

**数据转换流程**:
```
原始数据
  ├─ 图像 → Resize(224, 224) → ToTensor → Normalize
  ├─ 文本 → 直接传递（在模型中 tokenize）
  └─ BBox [x1,y1,x2,y2] → 动作向量 [cx,cy,scale]
                           cx = (x1 + x2) / 2 / width
                           cy = (y1 + y2) / 2 / height
                           scale = sqrt(area) / sqrt(image_area)
```

**使用方式**:
```python
from src.dataset import create_dataloaders

train_loader, val_loader = create_dataloaders(
    batch_size=4,
    num_workers=4,
    test_split=0.1,
)

for batch in train_loader:
    images_t1 = batch['image_t1']  # (B, 3, 224, 224)
    images_t2 = batch['image_t2']  # (B, 3, 224, 224)
    captions = batch['caption']    # List[str]
    actions = batch['action_vector']  # (B, 3)
```

---

### 3. model.py - 模型架构

**功能**:
- CLIP 视觉编码器（冻结）
- Qwen LLM（LoRA 微调）
- 特征融合
- 动作预测头

**模型架构**:
```python
class VLM_ActionAgent(nn.Module):
    def __init__(self):
        # 1. CLIP 视觉编码器（冻结）
        self.clip_model = CLIPModel.from_pretrained(...)
        for param in self.clip_model.vision_model.parameters():
            param.requires_grad = False

        # 2. LLM（4位量化 + LoRA）
        bnb_config = BitsAndBytesConfig(load_in_4bit=True, ...)
        self.llm_model = AutoModelForCausalLM.from_pretrained(
            ..., quantization_config=bnb_config
        )

        # 应用 LoRA
        lora_config = LoraConfig(r=8, lora_alpha=16, ...)
        self.llm_model = get_peft_model(self.llm_model, lora_config)

        # 3. 投影层
        self.projector = nn.Linear(512, 1024)

        # 4. 动作预测头
        self.action_head = nn.Sequential(
            nn.Linear(1024, 256),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(128, 3),
            nn.Sigmoid()  # 归一化到 [0, 1]
        )
```

**前向传播流程**:
```
输入: images_t1, images_t2, captions
  ↓
视觉编码:
  vision_feat_t1 = clip_model.vision_model(images_t1)  # (B, 512)
  vision_feat_t2 = clip_model.vision_model(images_t2)  # (B, 512)
  temporal_feat = concat([vision_feat_t1, vision_feat_t2])  # (B, 1024)
  projected_feat = projector(temporal_feat)  # (B, 1024)
  ↓
语言编码:
  tokens = tokenizer(captions)
  llm_out = llm_model(tokens)
  llm_feat = llm_out.hidden_states[-1][:, 0, :]  # (B, 1024)
  ↓
特征融合:
  fused_feat = projected_feat + llm_feat  # (B, 1024)
  ↓
动作预测:
  action_pred = action_head(fused_feat)  # (B, 3)
  ↓
输出: action_pred = [cx, cy, scale] ∈ [0, 1]³
```

**参数统计**:
```python
总参数: ~500M
可训练参数: ~2M (LoRA + 投影层 + 动作头)
冻结参数: ~498M (CLIP 视觉编码器)
训练比例: ~0.4%
```

---

### 4. train.py - 训练流程

**功能**:
- 完整训练循环
- 混合精度训练
- 梯度累积和裁剪
- 检查点保存
- 指标记录

**Trainer 类**:
```python
class Trainer:
    def __init__(self, config):
        self.config = config
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.global_step = 0
        self.epoch = 0
        self.best_loss = float('inf')

    def load_data(self):
        # 加载训练和验证数据
        self.train_loader, self.val_loader = create_dataloaders(...)

    def setup_model(self):
        # 初始化模型、优化器、调度器
        self.model = create_model(...)
        self.optimizer = optim.AdamW(...)
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(...)
        self.scaler = GradScaler()  # 混合精度

    def train_epoch(self):
        # 一个 epoch 的训练
        for batch in self.train_loader:
            # 前向传播（混合精度）
            with autocast(dtype=torch.bfloat16):
                outputs = self.model(...)
                loss = outputs['total_loss']

            # 反向传播（梯度缩放）
            self.scaler.scale(loss).backward()

            # 梯度裁剪和优化器步进
            self.scaler.unscale_(self.optimizer)
            torch.nn.utils.clip_grad_norm_(..., max_norm=1.0)
            self.scaler.step(self.optimizer)
            self.scaler.update()

    def validate(self):
        # 验证循环
        with torch.no_grad():
            for batch in self.val_loader:
                outputs = self.model(...)
                # 计算验证损失

    def save_checkpoint(self, step, is_best=False):
        # 保存检查点
        checkpoint = {
            'epoch': self.epoch,
            'global_step': self.global_step,
            'model_state_dict': self.model.state_dict(),
            'optimizer_state_dict': self.optimizer.state_dict(),
            'metrics': self.metrics,
        }
        torch.save(checkpoint, f"checkpoint_step_{step}.pt")
```

**训练循环**:
```
for epoch in range(MAX_EPOCHS):
    ├─ train_epoch():
    │   ├─ 前向传播 (混合精度)
    │   ├─ 计算损失
    │   ├─ 反向传播 (梯度累积)
    │   ├─ 梯度裁剪
    │   ├─ 优化器步进
    │   └─ 学习率调度
    ├─ validate():
    │   └─ 评估验证集
    └─ save_checkpoint():
        ├─ 定期保存 (每 500 步)
        └─ 保存最佳模型
```

---

### 5. utils.py - 工具函数

**主要函数**:

| 函数 | 功能 |
|------|------|
| `set_seed(seed)` | 设置随机种子 |
| `get_device()` | 获取 GPU/CPU 设备 |
| `count_parameters(model)` | 统计模型参数 |
| `print_gpu_memory()` | 显示 GPU 内存 |
| `load_checkpoint(path, model)` | 加载检查点 |
| `format_time(seconds)` | 格式化时间 |
| `AverageMeter` | 计算平均值 |

**使用示例**:
```python
from src.utils import (
    set_seed,
    get_device,
    print_model_summary,
    print_gpu_memory
)

# 设置随机种子
set_seed(42)

# 获取设备
device = get_device()

# 打印模型摘要
print_model_summary(model)

# 监控 GPU 内存
print_gpu_memory()
```

---

## 数据流程

### Arrow 数据集结构

```
LEVIR-CC/
├── dataset_info.json           # 数据集元信息
├── train/
│   ├── data-00000.arrow       # 训练数据
│   └── state.json
└── validation/
    ├── data-00000.arrow       # 验证数据
    └── state.json
```

### 数据样本格式

```python
{
    'image': PIL.Image (H, W, 3),      # 时间1影像
    'image2': PIL.Image (H, W, 3),     # 时间2影像
    'caption': str,                     # "building area increased"
    'bbox': [x1, y1, x2, y2],          # [100, 150, 300, 350]
}
```

### 数据预处理流程

```
1. 加载 Arrow 数据
   ├─ datasets.load_from_disk(path)
   └─ 自动检测数据结构

2. 图像预处理
   ├─ Resize → (224, 224)
   ├─ ToTensor → (3, 224, 224)
   └─ Normalize → CLIP 标准化

3. BBox 转换
   ├─ 计算中心点: cx = (x1+x2)/(2*W)
   ├─ 计算中心点: cy = (y1+y2)/(2*H)
   └─ 计算尺度: scale = sqrt(area/total_area)

4. 批次组装
   └─ DataLoader → (B, ...)
```

---

## 训练流程

### 完整训练管道

```
1. 初始化阶段
   ├─ 加载配置
   ├─ 验证路径
   ├─ 创建输出目录
   └─ 设置随机种子

2. 数据加载
   ├─ 加载 Arrow 数据集
   ├─ 划分训练/验证集 (9:1)
   └─ 创建 DataLoader

3. 模型初始化
   ├─ 加载 CLIP（冻结）
   ├─ 加载 Qwen（4位量化）
   ├─ 应用 LoRA
   └─ 初始化动作预测头

4. 优化器设置
   ├─ AdamW 优化器
   ├─ 余弦退火调度器
   └─ 混合精度 Scaler

5. 训练循环 (每个 epoch)
   ├─ 训练阶段
   │   ├─ 前向传播
   │   ├─ 计算损失
   │   ├─ 反向传播
   │   ├─ 梯度裁剪
   │   └─ 优化器步进
   ├─ 验证阶段
   │   ├─ 评估验证集
   │   └─ 计算验证损失
   └─ 检查点保存
       ├─ 定期保存 (每 500 步)
       └─ 最佳模型保存

6. 训练完成
   ├─ 保存最终检查点
   ├─ 保存训练指标
   └─ 打印训练总结
```

### 损失函数

```python
# 动作预测损失（MSE）
action_loss = MSELoss(action_pred, action_target)

# 总损失
total_loss = action_loss
```

### 学习率调度

```python
# 余弦退火
scheduler = CosineAnnealingLR(
    optimizer,
    T_max=total_steps,
    eta_min=lr * 0.1
)

# 学习率变化曲线
lr_start = 1e-4
lr_min = 1e-5
lr(t) = lr_min + (lr_start - lr_min) * (1 + cos(πt/T_max)) / 2
```

---

## 配置详解

### 训练超参数

```python
# 基本设置
MAX_EPOCHS = 10              # 训练轮数
BATCH_SIZE = 4               # 批次大小
LEARNING_RATE = 1e-4         # 学习率
WEIGHT_DECAY = 0.01          # 权重衰减

# 优化设置
MAX_GRAD_NORM = 1.0          # 梯度裁剪
WARMUP_STEPS = 500           # 预热步数
GRADIENT_ACCUMULATION_STEPS = 1  # 梯度累积

# 保存和评估
SAVE_INTERVAL = 500          # 保存间隔
EVAL_INTERVAL = 1000         # 评估间隔
LOG_INTERVAL = 100           # 日志间隔
```

### 模型超参数

```python
# CLIP 设置
VISION_MODEL_NAME = "openai/clip-vit-base-patch32"
VISION_HIDDEN_DIM = 512
VISION_OUTPUT_DIM = 512

# Qwen 设置
LLM_MODEL_NAME = "Qwen/Qwen2.5-0.5B"
LLM_HIDDEN_DIM = 1024
LLM_NUM_LAYERS = 24

# LoRA 设置
LORA_R = 8                   # LoRA 秩
LORA_ALPHA = 16              # 缩放因子
LORA_DROPOUT = 0.05          # Dropout
LORA_TARGET_MODULES = ["q_proj", "v_proj"]  # 目标层

# 量化设置
USE_4BIT = True              # 使用 4 位量化
COMPUTE_DTYPE = "bfloat16"   # 计算精度
```

### 数据超参数

```python
IMAGE_SIZE = 224             # CLIP 输入尺寸
MAX_TEXT_LENGTH = 128        # 文本最大长度
BBOX_NORMALIZE = True        # 边界框归一化
NUM_WORKERS = 4              # DataLoader 工作进程数
```

---

## 性能优化

### 内存优化技术

1. **4位量化**
```python
# 减少 LLM 内存占用 75%
bnb_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_use_double_quant=True,
    bnb_4bit_quant_type="nf4"
)
```

2. **LoRA 微调**
```python
# 只训练 0.4% 的参数
lora_config = LoraConfig(r=8, lora_alpha=16)
model = get_peft_model(model, lora_config)
```

3. **混合精度训练**
```python
# 使用 bfloat16 减少内存和加速
with autocast(dtype=torch.bfloat16):
    outputs = model(...)
```

4. **梯度检查点**
```python
# 用计算换内存
model.gradient_checkpointing_enable()
```

### 训练加速技术

1. **梯度累积**
```python
# 模拟更大的批次
GRADIENT_ACCUMULATION_STEPS = 4
effective_batch_size = BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS
```

2. **数据加载优化**
```python
# 多进程数据加载
DataLoader(..., num_workers=8, pin_memory=True)
```

3. **编译优化**
```python
# PyTorch 2.0 编译
model = torch.compile(model)
```

---

## 扩展开发

### 添加新的数据集

1. **实现自定义 Dataset**:
```python
class CustomDataset(Dataset):
    def __init__(self, data_path):
        # 加载数据
        pass

    def __getitem__(self, idx):
        return {
            'image_t1': ...,
            'image_t2': ...,
            'caption': ...,
            'action_vector': ...,
        }
```

2. **修改 dataset.py**:
```python
def create_custom_dataloaders(...):
    dataset = CustomDataset(...)
    loader = DataLoader(dataset, ...)
    return loader
```

### 修改模型架构

1. **更换视觉编码器**:
```python
# 替换 CLIP
from transformers import ViTModel
self.vision_model = ViTModel.from_pretrained("google/vit-base-patch16-224")
```

2. **更换语言模型**:
```python
# 替换 Qwen
from transformers import AutoModelForCausalLM
self.llm_model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b")
```

3. **修改融合方式**:
```python
# 从相加改为拼接 + MLP
fused_feat = torch.cat([vision_feat, llm_feat], dim=-1)  # (B, 2048)
fused_feat = self.fusion_mlp(fused_feat)  # (B, 1024)
```

### 添加新的损失函数

```python
# 在 model.py 中添加
class VLM_ActionAgent(nn.Module):
    def __init__(self):
        # ...
        self.focal_loss = FocalLoss()

    def forward(self, ...):
        # ...
        if action_targets is not None:
            action_loss = self.focal_loss(action_pred, action_targets)
            # 或组合多个损失
            total_loss = action_loss + 0.1 * auxiliary_loss
```

---

## 🎓 最佳实践

### 训练建议

1. **先小规模测试**
```python
# 在 config.py 中
DEBUG = True  # 只训练 5 个批次
BATCH_SIZE = 2  # 小批次测试
```

2. **监控 GPU 内存**
```python
from src.utils import print_gpu_memory
print_gpu_memory()  # 定期检查
```

3. **保存配置**
```python
from src.utils import save_config_as_json
save_config_as_json(Config, "config.json")
```

4. **版本控制**
```bash
git commit -m "Training run with lr=1e-4"
```

### 调试技巧

1. **使用 DEBUG 模式**
```python
Config.DEBUG = True  # 快速迭代
```

2. **打印模型结构**
```python
from src.utils import print_model_summary
print_model_summary(model)
```

3. **验证数据**
```python
# 可视化一个批次
batch = next(iter(train_loader))
print(batch['image_t1'].shape)
print(batch['caption'][0])
```

---

## 📚 参考资源

### 核心论文
- CLIP: https://arxiv.org/abs/2103.14030
- Qwen: https://arxiv.org/abs/2309.16609
- LoRA: https://arxiv.org/abs/2106.09685

### 工具文档
- PyTorch: https://pytorch.org/docs
- Transformers: https://huggingface.co/docs/transformers
- PEFT: https://huggingface.co/docs/peft

---

[返回 README](./README.md) | [查看快速开始](./快速开始.md)

